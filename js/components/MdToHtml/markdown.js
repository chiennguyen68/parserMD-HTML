...
// const { parseBoolean, parseMaxHeader } = require("./lib.js");
...
const REGEX_ESCAPE_CHAR = new RegExp(`\\${MD_ESCAPE_CHAR}([${MD_CHARS}])`, "g");

const removeEscapeChars = (text) =>
  text.replace(REGEX_ESCAPE_CHAR, (match, char) => char);

const REGEX_MD_TEXT = new RegExp(`[${MD_CHARS}]`);
...
export const parse = (markdownText, opt = {}) => {
  ...
  while (cursor < textSize) {
    ...
    if (lineText.trim().length === 0) {
      if (currentNode != null && currentNode.tagName === "P") {
        flushBody();
      }
    } else {
      lineCursor = 0;
      ...
      if (firstChar === MD_ESCAPE_CHAR) {
        lineCursor = 1;
      } else if (allowHeader && firstChar === "#") {
        let headerLevel = 1;

        while (headerLevel < maxHeader && next(headerLevel) === "#") {
          headerLevel += 1;
        }
        ...
        if (next(1) === "[" && allowImage) {
          ...
          if (endMatch) {
            ...
            if (allowHTMLAttributes && attrs != null) {
        ...
        if (
          ...
          parseLine = false;
          flushBody();
        } else if (allowUnorderedList && next(1) === " ") {
          if (currentNode != null && currentNode.tagName !== "UL") {
            flushBody();
          }
        ...
        if (match) {
          ...
          if (currentNode != null && currentNode.tagName !== "OL") {
            flushBody();
          }
        ...
        const newListTag = itemMatch[2] === "- " ? "UL" : "OL";
        ...
        if (
          ...
          if (listLevel === 2 && targetNode.parentNode.tagName === newListTag) {
            ...
            listNode.onAttach =
              newListTag === "UL" ? opt.onUnorderedList : opt.onOrderedList;
            listNode.appendChild(document.createElement("LI"));
      ...
      } else if (allowQuote && firstChar === ">") {
        if (next(1) === " ") {
          if (currentNode == null || currentNode.tagName !== "BLOCKQUOTE") {
            flushBody();
          }
      ...
      } else if (allowMultilineCode && lineText.startsWith("```")) {
        const match = /^```(\w*)\n((.|\n(?!```))+)\n```/.exec(
      ...
      } else if (allowFootnote && firstChar === "[") {
        ...
        while (lineCursor <= EOLIndex) {
          ...
          if (match == null) {
            ...
            if (next(-1) === MD_ESCAPE_CHAR) {
              ...
              if (
                targetNode == null ||
                (targetNode.tagName !== "EM" && targetNode.tagName !== "STRONG")
              ) {
                const remainingText = lineText.substring(lineCursor);
                const match = /^(\*{1,3})(.*?\S)(\*{1,3})/.exec(remainingText);
